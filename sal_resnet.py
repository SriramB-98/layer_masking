...
#             print(out[1].shape, identity[1].shape)
#             print( (out[1] != identity[1]).float().mean())
            o_i, o_m, o_it = out
            o_m = (o_m*identity[1]).float()
            o_i += identity[0]
            o_i *= o_m
            out = (o_i, o_m, o_it)
...
class ResNet(nn.Module):
    def __init__(
        ...
        for m in self.modules():
            ...
            elif isinstance(m, (MyBatchNorm2d, nn.GroupNorm)):
