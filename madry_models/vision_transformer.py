...
#         print(x.shape)
    ...
    def forward(self, x):
        if isinstance(x, tuple):
            ...
            n_w = w // p
            mask = mask-1
            mask = mask.reshape(n, n_h, p, n_w, p).sum(dim=(-1, -3))
            mask = mask.reshape(n, n_h*n_w) >= 0
            
            mask_list = [torch.where(m)[0] for m in mask]
            max_len = max([len(x) for x in mask_list])
            mask = torch.stack([torch.cat((m, -2*torch.ones(max_len-len(m), device=m.device, dtype=m.dtype))) for m in mask_list])
            patch_mask = torch.cat([torch.zeros(n, 1, dtype=mask.dtype, device=mask.device), mask+1], dim=1)
        else:
            patch_mask = None
