    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.use_res_connect:
            ...
            if isinstance(input, Tensor):
                ...
                inp, mask, it = input
                res_inp, res_mask, it = result
                new_mask = mask*res_mask#torch.clip(mask+res_mask,0,1)
                result = ((res_inp+inp)*new_mask,new_mask,it)
        return result
...
class EfficientNet(nn.Module):
    def __init__(
        ...
        **kwargs: Any,
        ...
        layers.append(
            ConvNormActivation(
                3, firstconv_output_channels, kernel_size=3, stride=2, norm_layer=norm_layer, activation_layer=MySiLU
            )
        )
        ...
        self.avgpool = MyAdaptiveAvgPool2d(1)
...
def efficientnet_b5(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> EfficientNet:
    ...
    return _efficientnet(
        ...
        pretrained,
        progress,
        norm_layer=partial(MyBatchNorm2d, eps=0.001, momentum=0.01),
        **kwargs,
    )
